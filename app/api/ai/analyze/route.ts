...
function parseJson(text: string): AnalysisResult {
  ...
  const safeFields: Array<keyof Pick<AnalysisResult, 'color' | 'secondary_color' | 'pattern' | 'era' | 'style_key' | 'notes'>> =
    ['color', 'secondary_color', 'pattern', 'era', 'style_key', 'notes'];
  ...
  if (condition && allowedConditions.includes(condition) && confidenceFor(raw, 'condition') >= 0.65) result.condition = condition;
...
brand, category, subcategory, color, secondary_color, material, pattern, condition, season, original_size, size_system, de_size, international_size, era, style_key, occasions, measurements, flaws, notes, confidence.
